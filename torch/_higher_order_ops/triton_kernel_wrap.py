    ...
    def mlir_to_functions(op: "TritonIROperation") -> None:
        ...
        if name == "tt.func":
            ...
            if result_ids:
                ...
                block_ops[fake_res].append(Op(name, callee, args, fake_res, is_pure=is_pure))
        ...
        for op in op_list:
            ...
            if op.name in UNKNOWN_OPS:
                if op.name == "tt.elementwise_inline_asm" and op.is_pure:
                    log.warning(f"TTIR mutation analysis: Skipping pure tt.elementwise_inline_asm op (is_pure={op.is_pure})")
                    continue
                raise RuntimeError(
